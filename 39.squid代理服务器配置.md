...
![spt-squid-p04](http://imglf2.nosdn.127.net/img/MHBOZ3dkVUUrdWpwZE9YSnVza04rNXF6U1NhSnN0V3F0MXpaVlFqQnFLY2RnWDhVUmxjVUpnPT0.jpg?imageView&thumbnail=500x0&quality=96&stripmeta=0&type=jpg)




###### 使用方式为squid透明代理模式，对内网用户做上网的访问控制策略。

1.挑选squid 源码安装包  ----  squid-3.0.STABLE18.tar.gz

创建squid的非登录式用户
useradd -M -s /sbin/nologin squid   // 表示不创建家目录

tar xf squid-3.0.STABLE18.tar.gz -C /usr/src/squid-package

进入源码变异安装目录

cd /usr/src/squid-package

指定编译安装的参数：

./configure --prefix=/usr/local/squid --sysconfdir=/etc --enable-linux-netfilter --enable-linux-tproxy --enable-async-io=100 --enable-err-language="Simplify_Chinese" --enable-underscore --enable-poll --enable-gnuregex  --enable-arp-acl
